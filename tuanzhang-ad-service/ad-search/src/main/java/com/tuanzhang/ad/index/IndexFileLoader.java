...
public class IndexFileLoader {
    ...
    public void init(){
        ...
        List<String> adCreativeStings = loadDumpData(String.format(DConstant.DATA_ROOT_DIR, DConstant.AD_CREATIVE));
        adCreativeStings.forEach(p -> AdLevelDataHandler.handleLevel2(JSON.parseObject(p, AdCreativeTable.class), OpType.ADD));

        List<String> adUnitStings = loadDumpData(String.format(DConstant.DATA_ROOT_DIR, DConstant.AD_UNIT));
        adUnitStings.forEach(p -> AdLevelDataHandler.handleLevel3(JSON.parseObject(p, AdUnitTable.class), OpType.ADD));

        List<String> adCreativeUnitStrings = loadDumpData(String.format(DConstant.DATA_ROOT_DIR, DConstant.AD_CREATIVE_UNIT));
        adCreativeUnitStrings.forEach(p -> AdLevelDataHandler.handleLevel3(JSON.parseObject(p, AdCreativeUnitTable.class), OpType.ADD));

        List<String> adUnitDistrictStings = loadDumpData(String.format(DConstant.DATA_ROOT_DIR, DConstant.AD_UNIT_DISTRICT));
        adUnitDistrictStings.forEach(p -> AdLevelDataHandler.handleLevel4(JSON.parseObject(p, AdUnitDistrictTable.class), OpType.ADD));


        List<String> adUnitItStings = loadDumpData(String.format(DConstant.DATA_ROOT_DIR, DConstant.AD_UNIT_IT));
        adUnitItStings.forEach(p -> AdLevelDataHandler.handleLevel4(JSON.parseObject(p, AdUnitItTable.class), OpType.ADD));

        List<String> adUnitKeywordStings = loadDumpData(String.format(DConstant.DATA_ROOT_DIR, DConstant.AD_UNIT_KEYWORD));
